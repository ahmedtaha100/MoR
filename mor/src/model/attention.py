...
class KVCache:
    """Standard KV cache that stores all tokens."""
...
class SelectiveKVCache:
    """
    Selective KV cache that only stores K/V for active (selected) tokens.
    This implements true KV compaction as described in the MoR paper.
    """
    ...
    def update(
        ...
        """Fallback to full caching when no active_mask provided."""
        seq_len = key.shape[2]
        if self.key_cache is None:
            self.key_cache = key
            self.value_cache = value
    ...
    def update_selective(
        ...
        """
        Update cache with only the active (selected) tokens.

        Args:
            key: [batch, num_kv_heads, seq_len, head_dim]
            value: [batch, num_kv_heads, seq_len, head_dim]
            position_ids: [batch, seq_len] - original position IDs
            active_mask: [batch, seq_len] - True for tokens to cache

        Returns:
            Tuple of (cached_keys, cached_values, cached_positions)
        """
...
class MoRAttention(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            shared_kv: Optional tuple of (key_states, value_states) to reuse from r=0.
                       Used for shared KV strategy even without caching.

        Returns:
            Tuple of (attn_output, attn_weights, kv_cache, computed_kv)
            computed_kv is (key_states, value_states) for potential reuse in shared mode.
        """
